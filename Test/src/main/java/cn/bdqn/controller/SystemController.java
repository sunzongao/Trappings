...
public class SystemController {
	 ...
	 * 查看角色拥有的权限
	 */
	@RequestMapping("/jsp/show_permission")
	public String showRole(int rid, Model model) {
		// 获取指定角色拥有的权限
		List<Permission> perlist = roleService.queryAllPerByRid(rid);
		// 获取所有1级权限集合
		List<Permission> p1 = permissionService.queryAllPer1();
	 ...
	 * @return
	 */
	@RequestMapping(value="jsp/renameflag",method = RequestMethod.POST, produces = "text/html;charset=UTF-8")
	@ResponseBody
	public String renameflag(String flag) {
		if(roleService.queryByflag(flag)!=null) {
	 ...
	 * @return
	 */
	@RequestMapping(value="jsp/renamername",method = RequestMethod.POST, produces = "text/html;charset=UTF-8")
	@ResponseBody
	public String renamername(String rname) {
		if(roleService.queryByRname(rname)>0) {
			return "false";
		}else {
			return "true";
		}
	 ...
	 * @return
	 */
	@RequestMapping("/jsp/addrole")
	@ResponseBody
	public String doaddrole(String flag,String rname,String id,boolean power) {
		String data="新增成功！";
		if(roleService.addrole(rname, flag)<=0) {
			data="1";
		}
		if(power) {
			String[] split = id.split(",");
			Role role = roleService.queryByflag(flag);
			for(int i=0;i<split.length;i++) {
				if(roleService.addRolePer(role.getId(),Integer.parseInt(split[i]))<=0) {
					data="2";
				}
			}
		}
