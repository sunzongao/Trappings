...
<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, role-scalable=no" />
...
<div class="margin" id="page_style">
   ...
   <form id="form1" action="${pageContext.request.contextPath}/jsp/roleList.html" method="post">
   	 <ul class="choice_search">
	     <li class="clearfix col-xs-2 col-lg-2 col-ms-3 "><label class="label_name ">角色名称：</label><input name="rname" value="${rname}" type="text"  class="form-control col-xs-6 col-lg-5"/></li>
   ...
   <tbody>
   	<c:forEach items="${util.lists}" var="role">
		 ...
		 <td class="td-status">${role.rname}</td>
	   	 <td class="td-manage">
            <a title="授权"  onclick="member_edit(this,${role.id})" href="javascript:;" class="btn button_btn bg-deep-blue">授权</a>        
            <a title="查看" href="javascript:;" onclick="Competence_View(this,'1')" class="btn button_btn btn-green">查看</a>
             <a title="查看" href="javascript:;" onclick="Competence_View(this,'1')" class="btn button_btn btn-green">新增角色</a>
		</td>
   	 </tr>
   	</c:forEach>
   </tbody>
  	...
  	<p style="margin-top: 5px;text-align: center;">
		<a href="${pageContext.request.contextPath}/jsp/roleList.html?pageindex=1&rname=${rname}" class="btn btn-xs btn-info" <c:if test="${util.currentPage==1}">style="display: none;"</c:if>>首页</a>&nbsp;&nbsp;
		<a href="${pageContext.request.contextPath}/jsp/roleList.html?pageindex=${util.currentPage-1}&rname=${rname}" class="btn btn-xs btn-info" <c:if test="${util.currentPage==1}">style="display: none;"</c:if>>上一页</a>&nbsp;&nbsp;
		<a href="${pageContext.request.contextPath}/jsp/roleList.html?pageindex=${util.currentPage+1}&rname=${rname}" class="btn btn-xs btn-info" <c:if test="${util.currentPage==util.totalPage}">style="display: none;"</c:if>>下一页</a>&nbsp;&nbsp;
		<a href="${pageContext.request.contextPath}/jsp/roleList.html?pageindex=${util.totalPage}&rname=${rname}" class="btn btn-xs btn-info" <c:if test="${util.currentPage==util.totalPage}">style="display: none;"</c:if>>尾页</a>
