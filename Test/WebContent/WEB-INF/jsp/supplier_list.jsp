...
<form action="" method="get">
   ...
   <div class="operation clearfix same_module mb15">
    <ul class="choice_search">
     <li class="clearfix col-xs-2 col-lg-2 col-ms-3 "><label class="label_name " style="width: 100px">供应商名称：</label><input name="" type="text"  class="form-control col-xs-6 col-lg-5"/></li>
  ...
  <table class="table  table_list table_striped table-bordered">
   ...
   <c:forEach items="${page.lists }" var="item" >
		<tr>
	     ...
	     <td><a href="javascript:" onClick="userinfo(this,121)">${item.sName }</a></td>
	     <td>${item.sContacts }</td>
	     <td>${item.sSex }</td>
	     <td>${item.sPhone }</td>
	     <td>${item.sEmail }</td>
	     <td>${item.sAddress }</td>
	     <td>${fn:substring(item.sDate,0,10)}</td>
	      <td class="td-status"><span class="label label-success radius">启用</span></td>
	        <td class="td-manage">
	        <a onClick="member_stop(this,'10001')"  href="javascript:;" title="停用"  class="btn btn-xs btn-status">停用</a> 
