...
bordered"
				...
				<thead>
					<tr>
						...
						<th>报损报溢时间</th>
						<th>数量</th>
						<th width="180">所属仓库</th>
						<th width="150">单位名称</th>
						<th>经办人姓名</th>
						<th>备注</th>
						<th width="200">操作</th>
					</tr>
					<c:forEach var="b" items="${pageUtil.lists }">

						<tr>
							<th><label><input type="checkbox" class="ace"><span
									class="lbl"></span></label></th>
							<th>${b.badoverflowId }</th>
							<th><c:if test="${b.type  eq  1}">报损 </c:if> <c:if
									test="${b.type  eq  2}">报溢 </c:if></th>
							<th>${b.badoverflowTime }</th>
							<th>${b.amount }</th>
							<th>${b.storehouseName }</th>
							<th>${b.unitName }</th>
							<th>${b.unitName }</th>
							<th>${b.comments }</th>
							<th><a href="javascript:void()"
								onclick="Advert_add(this,'+102+')"
								class="btn bg-deep-blue  operation_btn">添加</a> <a
								href="javascript:void()" onclick="picture_del(this,'+10001+')"
								class="btn btn-danger operation_btn">删除</a> <a
								href="advert_detailed_left.html"
								onclick="Advert_info(this,'+234+')"
								class="btn bg-deep-blue operation_btn">查看</a></th>
						</tr>
					</c:forEach>
				</thead>
				<tbody>
